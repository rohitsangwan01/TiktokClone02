...
import com.pvaindia.tiktokclone0.camera.ReferenceCamera
import com.pvaindia.tiktokclone0.camera.ReferenceCamera1
import com.pvaindia.tiktokclone0.camera.ReferenceCamera2
import com.pvaindia.tiktokclone0.common.PermissionHelper
import com.pvaindia.tiktokclone0.data.BeautyItemData
import com.pvaindia.tiktokclone0.databinding.ActivityCameraBinding
import com.pvaindia.tiktokclone0.model.ItemModel
import com.pvaindia.tiktokclone0.network.DownloadAsyncResponse
import com.pvaindia.tiktokclone0.network.DownloadAsyncTask
import com.pvaindia.tiktokclone0.rendering.CameraTexture
import com.pvaindia.tiktokclone0.rendering.ScreenRenderer
import com.pvaindia.tiktokclone0.util.FileDeleteAsyncTask
import com.pvaindia.tiktokclone0.util.FileDeleteAsyncTask.OnAsyncFileDeleteListener
import com.pvaindia.tiktokclone0.util.MediaStoreUtil
import com.pvaindia.tiktokclone0.util.PreferenceUtil
import com.pvaindia.tiktokclone0.viewmodel.ContentsViewModel
...
class CameraActivity : AppCompatActivity() {
    ...
    private var progressBarLimit: ProgressBar? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mDatabaseRef = mDatabase!!.reference.child("videos")

        progressBarLimit = findViewById(R.id.pb_video_limit)
...
//                dataBinding.shutterButton.setBackgroundResource(R.drawable.btn_shutter_video_purple)
//            }
    ...
    fun setSticker(item: ItemModel) {
        ...
        if (getLastUpdateAt(this@CameraActivity) > getStickerUpdateAt(
                this@CameraActivity,
                item.uuid ?: ""
            )) {
            FileDeleteAsyncTask(File(filePath), object : OnAsyncFileDeleteListener {
                override fun processFinish(result: Any?) {
                    ...
                    setStickerUpdateAt(
                        this@CameraActivity,
                        item.uuid ?: "",
                        getLastUpdateAt(this@CameraActivity)
                    )
    ...
    fun setFilter(item: ItemModel) {
        ...
        if (getLastUpdateAt(this@CameraActivity) > getFilterUpdateAt(
                this@CameraActivity,
                item.uuid ?: ""
            )) {
            FileDeleteAsyncTask(
    ...
    private fun takePictureOnGlThread(textureId: Int) {
        ...
        runOnUiThread {
            Toast.makeText(
                this@CameraActivity,
                "The file has been saved to your Gallery.",
                Toast.LENGTH_SHORT
            ).show()
    ...
    private fun startRecording() {
        ...
        videoFilePath = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            "$innerMediaPath/$videoId.mp4"
        } else {
            "$mediaPath/$videoId.mp4"
        }

//        videoFilePath = "/storage/emulated/0/Downloads/$videoId.mp4"
...
//            uploadVideoToFirebase(videoFilePath.toString())
...
//
//        val file = Uri.fromFile(File(videoPath))
//        val ref = mStorageRef!!.child("videos/$videoId.jpg")
...
//
//                val videoRef = mDatabaseRef!!.child(videoId.toString())
//                videoRef.child("id").setValue(videoId.toString())
//                videoRef.child("path").setValue(downloadUri.toString())
//
//            } else {
//                // Handle failures
//                // ...
...
//    }
    ...
    private fun videoLimitProgress() {
//        Handler().postDelayed({
//            stopRecording()
//        }, 15000)

        var i: Int = 0
        progressBarLimit?.progress = i

        progressBarLimit?.visibility = View.VISIBLE

        countDownTimer = object : CountDownTimer(15000, 100) {

            override fun onTick(millisUntilFinished: Long) {
                i++
                progressBarLimit?.progress = i * 100 / (15000 / 100)
            }

            override fun onFinish() {
                progressBarLimit?.progress = 100
                progressBarLimit?.visibility = View.INVISIBLE
                stopRecording()
            }
